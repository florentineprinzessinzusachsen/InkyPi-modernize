...
`.github/workflows/os-drift-nightly.yml` (daily cron) re-runs the install path against the **latest unpinned** `debian:trixie`/`bookworm` images — the unpinned complement to the pinned PR-gating install matrix. It catches upstream Debian/Pi OS package churn that a pinned matrix can't. Each leg adds the Raspberry Pi OS apt repo (same `[trusted=yes]` line as `Dockerfile.install-matrix`/`Dockerfile.sim-install` — a real Pi ships with it preconfigured, a bare `debian:<codename>` container does not) so Pi-only packages like `liblgpio-dev` resolve, then asserts every package in `install/debian-requirements.txt` resolves via `apt-cache show`, every pin in `install/requirements.txt` resolves via `pip install --dry-run`, and `scripts/install_testing/sim_install.sh` runs `install/install.sh` end-to-end inside a 512 MB arm64 sim of the Pi Zero 2 W. The `drift-check` job pins `shell: bash` at the job level — the container jobs' implicit default shell is `sh`, and bookworm's dash doesn't support `set -o pipefail` (only trixie's newer dash does). It has no `pull_request:` trigger — a broken nightly must never block merges. On failure it opens a GitHub issue labelled `os-drift`/`bug`. Manual runs: `workflow_dispatch` with an optional codename filter.

`bullseye` is deliberately not in this matrix (dropped 2026-08-15) — same JTN-615 rationale as `install-matrix.yml`: Debian 11 ships Python 3.9.2 against a `requirements.txt` that needs >=3.10, and its Raspberry Pi OS apt repo has never carried `chromium-headless-shell` or `liblgpio-dev`. Every nightly run failed identically on both legs from the workflow's first run — that's a permanently unsupported OS, not drift. `sim_install.sh`/`test_install_memcap.sh` still accept `bullseye` as a manual/diagnostic option (see their own scripts) — only the automated nightly matrix (and the `install-matrix.yml` PR gate) exclude it.
