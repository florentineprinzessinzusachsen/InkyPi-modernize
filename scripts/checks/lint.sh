...
MYPY_SRC_BASELINE_FILE="${MYPY_SRC_BASELINE_FILE:-${REPO_ROOT}/scripts/checks/mypy_src_baseline.txt}"
MYPY_TESTS_BASELINE_FILE="${MYPY_TESTS_BASELINE_FILE:-${REPO_ROOT}/scripts/checks/mypy_tests_baseline.txt}"
cd "${REPO_ROOT}" || exit

# Use existing environment in CI or when a virtualenv is active
if [[ -z "${CI:-}" && -z "${VIRTUAL_ENV:-}" ]]; then
    # shellcheck source=scripts/dev/venv.sh
    source scripts/dev/venv.sh
fi
...
# globstar makes scripts/**/*.sh recurse into the function-based subdirectories.
