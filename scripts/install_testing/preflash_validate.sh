...
if [[ ! -f ".venv/bin/activate" || ! -x ".venv/bin/python" ]]; then
    echo "FAIL venv      .venv is required; run 'scripts/dev/venv.sh' first" >&2
    exit 1
fi
...
import_smoke_check() {
    ...
    "${venv_python}" scripts/install_testing/preflash_smoke.py imports
}
...
soak_suite() {
    python scripts/install_testing/preflash_smoke.py soak
}
...
mutation_suite() {
    python scripts/checks/mutation_check.py
}
...
run_phase "app-smoke" python scripts/install_testing/preflash_smoke.py app
run_phase "render-smoke" python scripts/install_testing/preflash_smoke.py render
...
if is_truthy "${INKYPI_VALIDATE_PI_RUNTIME:-0}"; then
    if [[ "$(uname -s)" == "Linux" ]]; then
        run_phase "pi-runtime" python scripts/install_testing/preflash_smoke.py pi-runtime
    else
...
if is_truthy "${INKYPI_VALIDATE_BENCH_THRESHOLDS:-0}"; then
    run_phase "benchmarks" python scripts/install_testing/preflash_smoke.py benchmarks
else
...
if is_truthy "${INKYPI_VALIDATE_COLD_BOOT:-0}"; then
    run_phase "cold-boot" python scripts/install_testing/preflash_smoke.py cold-boot
else
    status_skip "cold-boot" "set INKYPI_VALIDATE_COLD_BOOT=1 to run cold-boot smoke"
fi

if is_truthy "${INKYPI_VALIDATE_CACHE:-0}"; then
    run_phase "cache" python scripts/install_testing/preflash_smoke.py cache
else
...
if is_truthy "${INKYPI_VALIDATE_BROWSER_RENDER:-0}"; then
    if [[ "$(uname -s)" == "Linux" ]]; then
        run_phase "browser" python scripts/install_testing/preflash_smoke.py browser-render
    else
...
if is_truthy "${INKYPI_VALIDATE_INSTALL_IDEMPOTENCY:-0}"; then
    if [[ "$(uname -s)" == "Linux" ]]; then
        run_phase "install-idem" python scripts/install_testing/preflash_smoke.py install-idempotency
