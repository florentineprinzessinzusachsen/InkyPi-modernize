...
def _catmull_rom_points(
    pts: list[tuple[float, float]], samples_per_seg: int = 16
...
def render_chart_svg(
    ...
    bottom_reserve = (
        round(height * 0.045)
        + round(label_font_size * 0.7)
        + round(legend_font_size * 2.7)
        + round(height * 0.02)
    )
    ...
    for i, label in enumerate(pct_labels):
        ...
        parts.append(
            f'<text x="{left - 8:.1f}" y="{y:.1f}" fill="{TEXT_COLOR}" font-size="{label_font_size}" '
    ...
    for j, raw in enumerate(intensities):
        ...
        bar_h = plot_h * frac
        parts.append(
            f'<rect x="{x - bar_w / 2:.1f}" y="{bottom - bar_h:.1f}" width="{bar_w:.1f}" '
    ...
    if len(smooth) >= 2:
        ...
        parts.append(
            f'<path d="{path_d}" fill="none" stroke="{LINE_COLOR}" stroke-width="{line_w}" '
            f'stroke-linejoin="round" stroke-linecap="round"/>'
        )
    dot_r = max(3, round(height * 0.006))
    for x, y in pts:
        parts.append(
            f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{dot_r}" fill="{LINE_COLOR}"/>'
        )
    ...
    step = (
        max(1, math.ceil((sample_label_w * 1.4) / point_spacing))
        if point_spacing
        else 1
    )
    ...
    for j in range(n):
        ...
        if j in label_indices:
            ...
            parts.append(
                f'<text x="{x:.1f}" y="{label_y:.1f}" fill="{TEXT_COLOR}" '
    ...
    swatch, item_pad, group_gap, intensity_w, probability_w, total_w = _legend_geometry(
        legend_font_size
    )
    if total_w > plot_w:
        legend_font_size = max(11, round(legend_font_size * plot_w / total_w * 0.95))
        swatch, item_pad, group_gap, intensity_w, probability_w, total_w = (
            _legend_geometry(legend_font_size)
        )
    ...
    if total_w <= plot_w:
        ...
        parts.append(
            f'<rect x="{gx:.1f}" y="{legend_y - swatch / 2:.1f}" width="{swatch}" height="{swatch}" fill="{BAR_COLOR}"/>'
        )
        parts.append(
            f'<text x="{gx + swatch + item_pad:.1f}" y="{legend_y:.1f}" fill="{TEXT_COLOR}" '
            f'font-size="{legend_font_size}" dominant-baseline="central">Intensity</text>'
        )
        gx2 = gx + swatch + item_pad + intensity_w + group_gap
        parts.append(
            f'<line x1="{gx2:.1f}" y1="{legend_y:.1f}" x2="{gx2 + swatch:.1f}" y2="{legend_y:.1f}" '
            f'stroke="{LINE_COLOR}" stroke-width="{line_w}"/>'
        )
        parts.append(
            f'<text x="{gx2 + swatch + item_pad:.1f}" y="{legend_y:.1f}" fill="{TEXT_COLOR}" '
        ...
        parts.append(
            f'<rect x="{gx1:.1f}" y="{legend_y - swatch / 2:.1f}" width="{swatch}" height="{swatch}" fill="{BAR_COLOR}"/>'
        )
        parts.append(
            f'<text x="{gx1 + swatch + item_pad:.1f}" y="{legend_y:.1f}" fill="{TEXT_COLOR}" '
            f'font-size="{legend_font_size}" dominant-baseline="central">Intensity</text>'
        )
        parts.append(
            f'<line x1="{gx2:.1f}" y1="{legend_y2:.1f}" x2="{gx2 + swatch:.1f}" y2="{legend_y2:.1f}" '
            f'stroke="{LINE_COLOR}" stroke-width="{line_w}"/>'
        )
        parts.append(
            f'<text x="{gx2 + swatch + item_pad:.1f}" y="{legend_y2:.1f}" fill="{TEXT_COLOR}" '
            f'font-size="{legend_font_size}" dominant-baseline="central">Probability</text>'
        )
