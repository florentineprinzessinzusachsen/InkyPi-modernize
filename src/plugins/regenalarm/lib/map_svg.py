...
def render_trajectory_svg(
    ...
    scale_y: float,
    hours: int = 4,
    unit_scale: float = 4.0,
    show_labels: bool = True,
    stroke_width: float = 3.0,
    crossbar_length_scale: float = 1.0,
    ...
    parts = [
        _arrowhead_svg(
            ...
            (loc_x, loc_y),
            width=stroke_width,
            head_len=14.0 * (stroke_width / 3.0),
    ...
    for k in range(1, hours + 1):
        ...
        half_len = (
            max(dist_from_loc * 0.23, k * 2.5 * avg_scale) * crossbar_length_scale
        )
        cx1, cy1 = tx + perp_x * half_len, ty + perp_y * half_len
        cx2, cy2 = tx - perp_x * half_len, ty - perp_y * half_len
        parts.append(
            f'<line x1="{cx1:.1f}" y1="{cy1:.1f}" x2="{cx2:.1f}" y2="{cy2:.1f}" '
        ...
        if show_labels:
            ...
            lx, ly = (
                tx + perp_x * label_offset * side,
                ty + perp_y * label_offset * side,
            )
...
def render_marker_and_trajectory(
    location_xy: tuple[int, int] | None,
    location_uv: tuple[float, float] | None,
    rain_native_size: tuple[int, int] | None,
    show_labels: bool = True,
    unit_scale: float = 4.0,
    stroke_width: float = 3.0,
    crossbar_length_scale: float = 1.0,
    ...
    parts = [
        f'<circle cx="{loc_x:.1f}" cy="{loc_y:.1f}" r="{r}" fill="{LOCATION_COLOR}"/>'
    ]
    ...
    if location_uv is not None:
        parts.append(
            render_trajectory_svg(
                ...
                crossbar_length_scale=crossbar_length_scale,
