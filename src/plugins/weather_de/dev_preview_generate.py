"""Dev-only tool: regenerates two files -
  dev_preview_inner.html - the actual render of weather_de.html, using the
    ...
    via file:// paths rather than inlining a frozen copy - so editing
    render/weather_de.css and reloading applies the change immediately, no
    need to re-run this script for CSS-only tweaks.
